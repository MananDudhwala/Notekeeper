...
export default function Dashboard(props) {
    ...
    const fetchImgFromUnsplash = async () => {
        ...
        await fetch(`https://api.unsplash.com/search/photos?query=${query[Math.floor(Math.random() * 10)]}&orientation=landscape&client_id=${apiKey}`)
            ...
            .then(data => {
                ...
                localStorage.setItem('BgImgData', JSON.stringify({ date: new Date().toLocaleDateString(), imgUrl: imageURL }));
            })
    ...
    return (
        <main id="container">
            ...
            <header style={{ background: `url(${bgImg}) center/cover no-repeat` }} className={`${welcomeContShrinked ? 'jumbotronShrinked' : ''} bgImgBackdropFilter`}>
                <div>
                    <p>Good {timeOfDay}, {props.username?.split(' ')[0]} 🤗</p>
                    <p>{new Date().toDateString().split(' ')[0]}, <span className='numberFont'>{new Date().toLocaleDateString()}</span></p>
                </div>
            </header>
            ...
            <div id="body" className={welcomeContShrinked ? 'full' : ''}>
                ...
                <nav>
                    ...
                    <div id="right">
                        <button className={`${!welcomeContShrinked ? 'rotate' : ''} btns`} onClick={() => setWelcomeContShrinked(!welcomeContShrinked)}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
