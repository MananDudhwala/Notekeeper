...
export default function SignupPage(props) {
    ...
    let saveLoginInfo = (user) => {
        ...
        let temp = JSON.parse(localStorage.getItem('SavedLoginArray')) || [];

        temp.push({ email: window.btoa(user.email), avatar: user.avatar, uid: window.btoa(user.key) })

        localStorage.setItem('SavedLoginArray', JSON.stringify(temp));
    }
    ...
    const sendVerificationEmail = (user) => {
        ...
        emailjs.send('service_iypi5jr', "template_s9z4kup", {
            ...
            .then(() => {
                ...
                localStorage.setItem('Notekeeper-AccountVerifyLinkData', JSON.stringify({ date: new Date().toLocaleDateString(), email: window.btoa(user.email), key: window.btoa(key) }))
            }).catch(error => {
                console.error(error);
        ...
        <div id='signupPage'>
            ...
            <div id="signupRight">
                ...
                <div id="signupBox">
                    ...
                    <button id="signupRememberBtn" onClick={() => setRememberMeBtn(!rememberMeBtn)}>
                        {rememberMeBtn ?
                    ...
                    <button id="main" className="signupBtns" onClick={createUser}>Sign Up</button>
                ...
                <button className="signupBtns" onClick={() => navigate('/home')}>Try as a Guest</button>
            </div>
        </div >
    )
}
