...
export default function CreateCanvasPage(props) {
    ...
    const addIconProp = (event) => {
        ...
        if (svgElement) {
            ...
            document.getElementById('canvasAndPropsContainer').appendChild(newShape);
    ...
    const addTextProp = (text) => {
        ...
        document.getElementById('canvasAndPropsContainer').appendChild(newText);
    ...
    const addImageProp = (imageToUse) => {
        ...
        document.getElementById('canvasAndPropsContainer').appendChild(newImage);
    ...
    const saveCanvas = () => {
        ...
        domtoimage.toPng(document.getElementById("canvasAndPropsContainer"))
    ...
    return (
        <div id="createCanvasPage">

            {loading && <Loading use={loading} />}

            <CSSTransition in={!isCanvasShow} timeout={500} classNames="zoomIn" unmountOnExit>
                <div id="createcanvasPage_canvasNameandBgCont">
                    <div id="canvasNameandBgCont">

                        <div id="canvasNameandBgCont_head">
                            <span id="canvasNameandBgCont_headLeft">
                                <button className="btns" onClick={() => navigate(window.history.length > 1 ? -1 : '/home')}>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                                        <path fillRule="evenodd" d="M11.354 1.646a.5.5 0 0 1 0 .708L5.707 8l5.647 5.646a.5.5 0 0 1-.708.708l-6-6a.5.5 0 0 1 0-.708l6-6a.5.5 0 0 1 .708 0z" />
                                    </svg>
                                </button>
                            </span>
                            <span id="canvasNameandBgCont_headMiddle">
                                <input value={canvasName} autoFocus spellCheck="false" autoComplete="off" type="text" className="input" placeholder='Title' onChange={(e) => setCanvasName(e.target.value)} onKeyUp={e => e.key === "Enter" && canvasName.trim().length > 2 && setIsCanvasShow(true)} />
                            </span>
                            <span id="canvasNameandBgCont_headRight"></span>
                        </div>

                        <div id="canvasNameandBgCont_body">
                        ...
                        <div id="canvasNameandBgCont_foot">
            ...
            <div id="createCanvasPage_body" onMouseMove={handleBodyMouseMove} onTouchMove={handleBodyMouseMove} onMouseUp={handleBodyMouseMoveStop} onTouchEnd={handleBodyMouseMoveStop} onMouseLeave={handleBodyMouseMoveStop}>
                ...
                <div id="canvasAndPropsContainer" style={canvasBgState === 'color' ? { backgroundColor: canvasBg } : canvasBgState === 'img' && canvasBgImg && { backgroundImage: `url(${canvasBgImg})` }} className={toolboxState}>

                    <div id="canvasContainer" onClick={unSelectProp} onTouchEnd={unSelectProp} style={{ '--height': `${fakeCursorHeight - 2}px`, '--width': `${fakeCursorHeight - 2}px`, '--top': `${fakeCursorTop}px`, '--left': `${fakeCursorLeft}px`, '--borderColor': fakeCursorBorderColor }}>

                        <canvas ref={canvasRef} id="canvas" className={`cursor-${toolboxState}`} height={document.body.clientHeight} width={document.body.clientWidth}></canvas>
