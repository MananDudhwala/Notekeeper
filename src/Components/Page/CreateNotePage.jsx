...
export default function CreateNotePage(props) {
    ...
    return (
        <>
            ...
            <div id="createNotePage" className={`${isBackground ? "bgImgBackdropFilter" : ""}`} style={isBackground ? { backgroundImage: `url(${backgroundImg})` } : null}>

                <section id="cnpHead">
                    <div id="cnphLeft">
                        <button className="btns" onClick={handleExitBtn}>
                    ...
                    <div id="cnphMiddle">
                        <input value={noteName} autoComplete='off' autoFocus type="text" className='input' placeholder='Name' onChange={(e) => setNoteName(e.target.value)} onKeyUp={(e) => e.key === 'Enter' && reactQuillRef.current?.focus()} />
                    </div>
                    <div id="cnphRight">
                        {recognition && window.navigator.onLine &&
                            <button id="micBtn" className={`btns ${isListening ? 'active' : ''}`} onClick={handleSpeechToText} >
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                        ...
                        {(plainText.trim().length > 0 && noteName.trim().length > 0) ?
                            <button className="btns" onClick={handleSaveBtn}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                ...
                <section id="cnpBody">
                    {isListening && <div id="interimTranscriptCont"><div id="interimTranscriptBox">{transcript}</div></div>}
                ...
                <section id="cnpFoot">
                    <div id="cnpfLeft">
                        <span id="cnpfLettersCount">Letters :
                            <span className="numberFont">{plainText.trim().length}</span>
                        </span>
                        <span id="cnpfWordsCount">Words :
                            <span className="numberFont">{plainText?.trim().length ? plainText?.trim().split(/\s+/).length : 0}</span>
                        </span>
                    </div>
                    {recognition && window.navigator.onLine &&
                        <button id="micBtn" className={`btns ${isListening ? 'active' : ''}`} onClick={handleSpeechToText} >
                    ...
                    <div id="cnpfRight">
                        Time to Read :
                        <span className="numberFont">{Math.floor(plainText.trim().split(/\s+/).length / 100)} mins</span>
                    </div>
                </section>
            </div>
        </>
