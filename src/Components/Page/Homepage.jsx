...
export default function Homepage(props) {
    ...
    useEffect(() => {
        ...
        changeAppSetting("AppColor", JSON.parse(localStorage.getItem('Notekeeper-AppColor')) || "96f1b5");

        // Set the Midnight Mode from user's choice, if chosen, otherwise set the Midnight mode to on.
        changeAppSetting("MidnightMode", JSON.parse(localStorage.getItem('Notekeeper-MidnightMode') || true));

        // Set the Auto Dark-Mode from user's choice, if chosen, otherwise set it to true.
        changeAppSetting("AutoDarkMode", JSON.parse(localStorage.getItem('Notekeeper-AutoDarkMode') || true));

        // Set the note background of user's choice, if any, otherwise set it to none.
        changeAppSetting("NoteBg", localStorage.getItem('Notekeeper-NoteBg') || 'none');

        // Set the custom background of user's choice, if uploaded, otherwise don't set anything.
        changeAppSetting("CustomNoteBg", localStorage.getItem('Notekeeper-CustomNoteBg') || '');
    }, []);
    ...
    const changeAppSetting = (type, update) => {
        if (type === "AppColor") {
            ...
            document.getElementById('tech').style.setProperty('--secondary', `#${update}`);

            setAppColor(update);
            localStorage.setItem('Notekeeper-AppColor', JSON.stringify(update));
        } else if (type === "AutoDarkMode") {
            setAutoDarkMode(update);
            localStorage.setItem('Notekeeper-AutoDarkMode', JSON.stringify(update));
            ...
            localStorage.setItem('Notekeeper-MidnightMode', JSON.stringify(update));
        } else if (type === "NoteBg") {
            setNoteBg(update);
            localStorage.setItem('Notekeeper-NoteBg', update);
        } else if (type === "CustomNoteBg") {
            setCustomNoteBg(update);
            localStorage.setItem('Notekeeper-CustomNoteBg', update);
        }
    }
    ...
    const syncOfflineData = async (notes, tasks, canvas, folder) => {
        ...
        const draftData = JSON.parse(localStorage.getItem('Notekeeper-DraftData')) || [];
        ...
        if (draftData.length && window.navigator.onLine) {
            try {
                ...
                localStorage.removeItem('Notekeeper-DraftData');
            } catch (err) {
    ...
    const syncGuestData = async () => {
        const guestNotesData = JSON.parse(localStorage.getItem('Notekeeper-GuestNotesData'));
        const guestTasksData = JSON.parse(localStorage.getItem('Notekeeper-GuestTasksData'));
        const guestCanvasData = JSON.parse(localStorage.getItem('Notekeeper-GuestCanvasData'));
        const guestFolderData = JSON.parse(localStorage.getItem('Notekeeper-GuestFolderData'));
        ...
        if ((guestNotesData || guestTasksData || guestCanvasData || guestFolderData) && window.navigator.onLine) {
            try {
                ...
                if (guestNotesData?.length) {
                    ...
                    localStorage.removeItem('Notekeeper-GuestNotesData');
                }
                ...
                if (guestCanvasData?.length) {
                    ...
                    localStorage.removeItem('Notekeeper-GuestCanvasData');
                }
                ...
                if (guestTasksData?.length) {
                    ...
                    localStorage.removeItem('Notekeeper-GuestTasksData');
                }
                ...
                if (guestFolderData?.length) {
                    ...
                    localStorage.removeItem('Notekeeper-GuestFolderData');
                }
    ...
    const fetchData = async (callback) => {
        if (window.navigator.onLine) {
            ...
            setNotesData(JSON.parse(localStorage.getItem('Notekeeper-Offline-Notes')) || []);
            setCanvasData(JSON.parse(localStorage.getItem('Notekeeper-Offline-Canvas')) || []);
        }
        callback();
    };

    const fetchGuestData = (callback) => {
        setNotesData(JSON.parse(localStorage.getItem('Notekeeper-GuestNotesData')) || []);
        setTasksData(JSON.parse(localStorage.getItem('Notekeeper-GuestTasksData')) || []);
        setCanvasData(JSON.parse(localStorage.getItem('Notekeeper-GuestCanvasData')) || []);
        setFolderData(JSON.parse(localStorage.getItem('Notekeeper-GuestFolderData')) || []);
        callback();
    }
        ...
        if (update === 'create') {
            ...
            if (props.user.authProvider === 'guest') {
                ...
                if (tempNotesData.length > 10) {
                    ...
                    localStorage.setItem('Notekeeper-GuestNotesData', JSON.stringify(tempNotesData))
                ...
                if (window.navigator.onLine) {
                    ...
                    localStorage.setItem('Notekeeper-DraftData', JSON.stringify(draftData));
                    callback();
                    props.updateNotification({ type: 'green', msg: 'Note Created Successfully.' });
            ...
            let offlineNotesData = JSON.parse(localStorage.getItem('Notekeeper-Offline-Notes')) || [];
            offlineNotesData = offlineNotesData.filter(el => el.key !== element.key);
            localStorage.setItem('Notekeeper-Offline-Notes', JSON.stringify(offlineNotesData));
            ...
            if (props.user.authProvider === 'guest') {
                ...
                if (window.navigator.onLine) {
                    ...
                    let draftData = JSON.parse(localStorage.getItem('Notekeeper-DraftData')) || [];
        ...
        if (update === 'create') {
            ...
            if (props.user.authProvider === 'guest') {
                if (tempTasksData.length > 10) {
                    ...
                    localStorage.setItem('Notekeeper-GuestTasksData', JSON.stringify(tempTasksData))
                    callback();
        ...
        if (update === 'create') {
            ...
            if (props.user.authProvider === 'guest') {
                if (tempCanvasData.length > 10) {
                    ...
                    localStorage.setItem('Notekeeper-GuestCanvasData', JSON.stringify(tempCanvasData))
                    callback();
            ...
            let offlineCanvasData = JSON.parse(localStorage.getItem('Notekeeper-Offline-Canvas')) || [];
            offlineCanvasData = offlineCanvasData.filter(el => el.key !== element.key);
            localStorage.setItem('Notekeeper-Offline-Canvas', JSON.stringify(offlineCanvasData));
            ...
            if (ind > -1) {
                ...
                if (props.user.authProvider === 'guest') {
                    localStorage.setItem('Notekeeper-GuestCanasData', JSON.stringify(tempCanvasData))
                } else if (window.navigator.onLine) {
        ...
        if (obj.use === 'create') {
            ...
            if (props.user.authProvider === 'guest') {
                ...
                if (tempCont.length > 10) {
                    ...
                    localStorage.setItem('Notekeeper-GuestFolderData', JSON.stringify(tempCont));
            ...
            if (props.user.authProvider === 'guest') {
                ...
                localStorage.setItem('Notekeeper-GuestNotesData', JSON.stringify(tempCont));
                obj.callBack && obj.callBack();
            ...
            {loading ?
                ...
                <Routes>
                    ...
                    <Route path="/setting/*" element={
                        <div id="settingPage" className="page">
                            <Sidebar user={props.user} appTheme={props.appTheme} updateTheme={props.updateTheme} />
                    ...
                    <Route path="/folder" element={
                        <div id='folderPage' className="page">
                            <Sidebar user={props.user} appTheme={props.appTheme} updateTheme={props.updateTheme} />
