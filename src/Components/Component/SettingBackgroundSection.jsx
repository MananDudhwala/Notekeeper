import { bgImgSmall } from '../Background'
...
export default function SettingBackgroundSection(props) {
  ...
  return (
    <div id="settingBackgroundSection">
      <div id="backgroundImgBox">

        {bgImgSmall.map((img, ind) => (
          <div className={`settingBgImgBoxes ${parseInt(props.noteBg) === ind ? 'selected' : ''}`} style={{ backgroundImage: `url(${img})` }} key={ind} onClick={() => props.changeAppSetting('NoteBg', ind)}>
            <span className="settingBgImgSelectedBox">
        ...
        <div id='noneBox' className={`settingBgImgBoxes ${props.noteBg === 'none' ? 'selected' : ''}`} onClick={() => props.changeAppSetting('NoteBg', 'none')}>
          <span id="settingBgImgNoneBox">
            {props.noteBg === 'none' ?
              <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 448 512"><path d="M448.1 118.2L437 129.7 173.6 404l-11.5 12-11.5-12L11.1 258.8 0 247.2l23.1-22.2 11.1 11.5L162.1 369.8 414 107.5 425 96l23.1 22.2z"></path></svg>
              :
        ...
        <div id='customBox' className={`settingBgImgBoxes ${props.noteBg === 'input' ? 'selected' : ''}`} style={{ backgroundImage: `url(${props.customNoteBg})` }}>

          <input type="file" title='' onChange={handleInputImage} />

          <div id="settingBgImgCustomBox">
