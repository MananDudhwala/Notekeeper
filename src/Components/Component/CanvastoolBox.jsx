...
export default function CanvastoolBox(props) {
    ...
    // Some collections for future use.
    const iconPack = [
    ...
    useEffect(() => {
        // Close the canvas toolbox on any touch outside of the it.
        document.getElementById('canvasAndPropsContainer').addEventListener('mousedown', () => { setIsToolBoxOpen(false); setHideToolBox(true) });
        document.getElementById('canvasAndPropsContainer').addEventListener('mouseup', () => setHideToolBox(false));
        document.getElementById('canvasAndPropsContainer').addEventListener('touchstart', () => { setIsToolBoxOpen(false); setHideToolBox(true) });
        document.getElementById('canvasAndPropsContainer').addEventListener('touchend', () => setHideToolBox(false));
        document.getElementById('canvasAndPropsContainer').addEventListener('mouseout', () => setHideToolBox(false));
    }, []);

    useEffect(() => {

        // Check if colorPickerRef.current exist and it has no childnodes then only proceed.
        if (colorPickerRef.current && colorPickerRef.current?.childElementCount === 0) {

            // Create a new instance of the iro color picker and pass the id of the element where we want to append it along with some custumization options.
            const colorPicker = new iro.ColorPicker("#canvasToolBox_colorPicker", {
                width: colorPickerRef.current.clientWidth - 15, // -15 so the color picker won't take the full width and there will be space to breath.
                color: pencilColor, //intital color of the color picker
                layout: [
                    {
            ...
            // Save the color picker instance for later use.
            setColorPickerState(colorPicker);

            // Add an event listener to the color picker so whenever this color is changed, it will change the color of the pencil.
            colorPicker.on('color:change', (clr) => setPencilColor(clr.hex8String));
        }
    }, [colorPickerRef.current])

    useEffect(() => {
        // Send the updated value of pencil size whenever it gets updated.
        props.pencilSizeChange(pencilSize);
    }, [pencilSize]);

    useEffect(() => {
        // Send the updated value of eraser size whenever it gets updated.
        props.eraserSizeChange(eraserSize);
    }, [eraserSize]);

    useEffect(() => {

        // Send the updated value of the pencil color whenever it gets updated. 
        props.pencilColorChange(pencilColor);

        // Update the value of the color picker whenever the pencil color updated.
        colorPickerState?.color.set(pencilColor);
    }, [pencilColor]);

    const deployEyeDropper = () => {
        // Check if the eye dropper exist in window object then only proceed.
        if ("EyeDropper" in window) {

            // Close the toolbox so the color picker cak easiy pick a color from the canvas.
            setIsToolBoxOpen(false);

            // Set a timeout of 100ms so the eye dropper will be dropped after the toolbox is closed.
            setTimeout(() => {
                new window.EyeDropper().open()
                    .then(clr => setPencilColor(clr.sRGBHex))
    ...
    const handlePropImg = (event) => {
        ...
        // Check if the selected image is cupported and the size of it is less then 3MB.
        if (selectedImage && selectedImage.size / (1024 * 1024) < 3) {
            ...
            // As soon as the data is ready, create a new image and store the data into it.
            reader.onload = (e) => {
                const img = new Image();
                img.src = e.target.result;

                // Send this image to the addImageProp function so it can be append over the canvas as a prop.
                props.addImageProp(img);

                // Close the toolbox after this process.
                setIsToolBoxOpen(false);

                // Change the state of the toolbox to move so the user can play with the image.
    ...
    const handleDrawingBtn = () => {
        ...
        // Check if the toolbox state is drawing. if yes then proceed otherwise set the toolbox state to draw.
        if (props.toolBoxState === 'draw') {

            // Check if the toolbox options component is opened as pencil options, if yes then close it otherwise open it. 
    ...
    const handleErasingBtn = () => {
        ...
        // Check if the toolbox state is erase. if yes then proceed otherwise set the toolbox state to erase. 
        if (props.toolBoxState === 'erase') {

            // Check if the toolbox option component is opened as erase or not. if yes then close it otherwise open it.
    ...
    const handleCustomIcon = (value) => {
        ...
        // Create a new button and append the custom icon into it.
        const newbtn = document.createElement('button');
        newbtn.classList.add('canvasToolBox_shapeBtn');
        newbtn.innerHTML = value;

        // Check if its a valid svg or not.
        if (newbtn.childNodes[0].nodeName === 'svg' && newbtn.childNodes[0].childElementCount) {

            const svgElement = newbtn.querySelector('svg');

            // Remove the default property of the custom svg icon.
            svgElement.removeAttribute('height');
            svgElement.removeAttribute('width');

            // Re-assign the color property of svg icon.
            svgElement.hasAttribute('stroke') && svgElement.setAttribute('stroke', '#808080')
            svgElement.setAttribute('fill', '#808080');

            // Remove all the color property from path so the color of the parent svg element will be applied everywhere inside of it.
            Array.from(svgElement.querySelectorAll('path')).forEach(node => {
                node.hasAttribute('fill') && node.removeAttribute('fill');
            });

            // Append this new button into the icons container
            document.getElementById('canvasToolBox_shapeOption_top').appendChild(newbtn);

            // Empty the custom icon input and hide it.
            setCustomIconsInputValue('');
            setCustomIconsInputShow(false);

            // Set the toolbox state to move and close the toolbox.
            props.handleToolBoxState('move');
            setIsToolBoxOpen(false);

            // Add an event listener to this button so everytime the user clicks on it, it will append the icon over the canvas.
            newbtn.onclick = props.addIconProp;

            // Automatically appent the icon over the canvas for the first time.
            newbtn.click();
        }
    }

    return (
        <div id="canvasToolBox" className={hideToolBox ? 'hide' : ''}>

            <div id="canvasToolBox_head">
                <button className="canvasToolBtns" id='toolBoxToggleBtn' onClick={() => setIsToolBoxOpen(!isToolBoxOpen)}>
                    <img src={appLogo} alt="App Logo" />
                </button>
            </div>

            <div id="canvasToolBox_body" className={isToolBoxOpen ? 'full' : ''}>

                <div id="canvasToolBox_bodyMain">

                    <button className={`${props.toolBoxState === 'draw' ? 'active ' : ''}canvasToolBtns`} id='toolBoxPencilBtn' onClick={handleDrawingBtn}>
                    ...
                    <button className={`${props.toolBoxState === 'erase' ? 'active ' : ''}canvasToolBtns`} id='toolBoxEraseBtn' onClick={handleErasingBtn}>
                    ...
                    <button className={`${props.toolBoxState === 'move' ? 'active ' : ''}canvasToolBtns`} id='toolBoxMoveBtn' onClick={() => { props.handleToolBoxState('move'); setIsToolBoxOpen(false) }}>
                    ...
                    <button className="canvasToolBtns" id='toolBoxUndoBtn' onClick={props.undoCanvas}>
                    ...
                    <button className="canvasToolBtns" id='toolBoxClearBtn' onClick={props.clearCanvas}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 512 512">
                    ...
                    <button className="canvasToolBtns" id='toolBoxImageBtn'>
                    ...
                    <button className={`canvasToolBtns`} id='toolBoxShapesBtn' onClick={() => { props.handleToolBoxState('move'); setToolOptions('shape') }}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 512 512">
                    ...
                    <button className="canvasToolBtns" id='toolBoxTextBtn' onClick={() => { props.handleToolBoxState('move'); setIsToolBoxOpen(false); props.addTextProp() }}>
                    ...
                    <button className="canvasToolBtns" id='toolBoxExitBtn' onClick={() => { props.updateWarning({ show: true, msg: `Your Canvas is not saved yet! Are you sure you want to exit?`, greenMsg: 'Cancel', redMsg: 'Exit', func: () => navigate(window.history.length > 1 ? -1 : '/home') }) }}>
                    ...
                    <button className="canvasToolBtns" id='toolBoxSaveBtn' onClick={props.saveCanvas}>
                ...
                <div id="canvasToolBox_bodySide">
                    <div id="canvasToolBox_drawingOption" className={`canvasToolBox_bodySideOptions ${toolOptions === 'pencil' ? 'active' : ''}`}>
                        <div id="canvasToolBox_sizeBox">
                            <div id="canvasToolBox_sizeCircleBox">
                                <div id="canvasToolBox_sizeCircle" style={{ height: `${pencilSize}px`, backgroundColor: pencilColor }}>
                                </div>
                            </div>
                            <input type="range" min={1} value={pencilSize} step={1} max={50} onChange={e => setPencilSize(e.target.value)} />
                        </div>
                        <div id="canvasToolBox_drawingColorBox">
                            <div id="canvasToolBox_drawingColorPalleteBox">
                                {colorPallete.map((color, index) => (
                                    <div key={index} className="canvasToolBox_colorPallete" style={{ backgroundColor: color }} onClick={() => setPencilColor(color)}>
                                    </div>
                                ))
                                }
                            </div>
                            <div id="canvasToolBox_drawingColorPickerBox">
                                <div id="canvasToolBox_eyeDropper">
                                ...
                                <div ref={colorPickerRef} id="canvasToolBox_colorPicker"></div>
                            </div>
                        </div>
                    </div>

                    <div id="canvasToolBox_eraseOption" className={`canvasToolBox_bodySideOptions ${toolOptions === 'erase' ? 'active' : ''}`}>
                        <div id="canvasToolBox_sizeBox">
                            <div id="canvasToolBox_sizeCircleBox">
                                <div id="canvasToolBox_sizeCircle" style={{ height: `${eraserSize}px`, width: `${eraserSize}px`, backgroundColor: 'black' }}>
                                </div>
                            </div>
                    ...
                    <div id="canvasToolBox_shapeOption" className={`canvasToolBox_bodySideOptions ${toolOptions === 'shape' ? 'active' : ''}`}>
                        <div id="canvasToolBox_shapeOption_top">
                            {shapePack.map((shape, index) => (
                                <button key={index} className='canvasToolBox_shapeBtn' onClick={() => { props.addShapeProp(shape); props.handleToolBoxState('move'); setIsToolBoxOpen(false); }}>
                                    <div className={`${shape}Shape`}></div>
                                </button>
                            ))}

                            {iconPack.map((icon, index) => (
                                <button key={index} className="canvasToolBox_shapeBtn" onClick={e => { props.addIconProp(e); props.handleToolBoxState('move'); setIsToolBoxOpen(false); }}>
                                    {icon}
                                </button>
                            ))}
                        </div>
                        <div id="canvasToolBox_shapeOption_bottom">
                            {customIconsInputShow ?
                                ...
                                :
                                <button id="canvasToolBox_shapeOption_default" onClick={() => setCustomIconsInputShow(true)}>
