...
export default function CanvasPropEdit(prop) {
        ...
        if (!selProp.classList.contains('imageProp')) {
            ...
            if (colorPickerRef.current && colorPickerRef.current?.childElementCount === 1) {
                ...
                colorPicker.on('color:change', (clr) => {
                    document.querySelector('#propEditPreview .selectedPropInner').style.setProperty('--propColor', clr.hex8String);
                });
            }
        }
    ...
    const updatePropAndExit = () => {
        ...
        prop.updateProp({
            ...
            width: propWidth,
            rotate: propRotate,
            color: document.querySelector('#propEditPreview .selectedPropInner').style.getPropertyValue('--propColor'),
            shadow: { x: textShadowX, y: textShadowY, blur: textShadowBlur, color: textShadowColor },
            prop: document.querySelector('#propEditPreview .selectedPropInner').childNodes[0]
        });
    ...
    const changePropSize = (val) => {
        ...
        if (propState === 'icon') {
            ...
            document.querySelector('#propEditPreview .selectedPropInner').style.setProperty('height', tempVal + 'px');
            document.querySelector('#propEditPreview .selectedPropInner').style.setProperty('width', tempVal + 'px');

        } else if (propState === 'text')
            document.querySelector('#propEditPreview .selectedPropInner p').style.fontSize = tempVal + 'px';

        else if (propState === 'image')
            document.querySelector('#propEditPreview .selectedPropInner img').style.width = tempVal + 'px';
    ...
    const changeShapeScale = (prop, val) => {
        ...
        if (prop === 'height') {
            ...
            document.querySelector('#propEditPreview .selectedPropInner').style.height = tempVal + 'px';
        }
        else {
            setPropWidth(tempVal);

            document.querySelector('#propEditPreview .selectedPropInner').style.width = tempVal + 'px';
        }
    }
    ...
    const handlePropShadow = (input, val) => {
        ...
        const selected = document.querySelector('#propEditPreview .selectedPropInner');
    ...
    const handleImageFilter = (input, val) => {
        ...
        const selected = document.querySelector('#propEditPreview .selectedPropInner img');
    ...
    const deployEyeDropper = () => {
        ...
        document.getElementById('canvasPropEdit').style.zIndex = -9;
    ...
    return (
        <div id="canvasPropEdit">

            {/* Button to PropEdit component  */}
            <button id="canvasPropEditExitBtn" onClick={updatePropAndExit}>
            ...
            <div id="canvasPropEditCont">

                {loading && <Loading use={'loading'} />}

                <div ref={propEditPreview} id="propEditPreview"></div>

                <div id="propEditOptions">

                    <div id="propEditOptionsTop">
                    ...
                    <div id="propEditOptionsBottom">

                        <div id="propEditBottom_default" className={editOption === 'none' ? 'active' : ''}>
                            <p>Choose an option to show more!</p>
                        </div>

                        <div id="propEditBottom_font" className={editOption === 'font' ? 'active' : ''}>
                            {fontArray.sort().map((font, index) => (
                                <button key={index} onClick={() => handlePropFontChange(font)}>
                                    <p id="propEditBottom_fontDemo" style={{ fontFamily: `${font}` }}>Aa</p>
                                    <p id="propEditBottom_fontName">{font}</p>
                                </button>
                            ))
                            }
                        </div>

                        <div id="propEditBottom_styleAndShadow" className={editOption === 'style' ? 'active' : ''}>
                            {propState !== 'icon' && propState !== 'shape' &&
                                <div id="propEditBottom_style">
                        ...
                        <div id="propEditBottom_frames" className={editOption === 'frame' ? 'active' : ''}>
                        ...
                        <div id="propEditBottom_size" className={editOption === 'size' ? 'active' : ''}>
                            {propState !== 'shape' ?
                                <>
                                    <div id="propEditBottom_sizeLeft">
                                        <input type="number" value={propSize} placeholder='Enter Size' onChange={(e) => changePropSize(e.target.value)} />
                                        px
                                    </div>
                                    <div id="propEditBottom_sizeRight">
                        ...
                        <div id="propEditBottom_rotate" className={editOption === 'rotate' ? 'active' : ''}>

                            <div id="propEditBottom_rotateLeft">
                                <input type="number" value={propRotate} placeholder='Rotate' onChange={(e) => changePropRotate(e.target.value)} />
                                deg
                            </div>

                            <div id="propEditBottom_rotateRight">
                        ...
                        <div id="propEditBottom_color" className={editOption === 'color' ? 'active' : ''}>

                            <div ref={colorPickerRef} id="propEditBottom_colorLeft">
                            ...
                            <div id="propEditBottom_colorRight">
